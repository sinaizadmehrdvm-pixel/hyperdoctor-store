...
if((map.match(/suggestedSiteSku:"HD-/g)||[]).length!==4)throw new Error("reconciliation suggestions must remain deliberately limited to four high-confidence matches");
const commerceFree=map.replace(/never carry price\/stock/i,"");
if(/\b(price|stock)\s*:/i.test(commerceFree))throw new Error("identity reconciliation map must not carry commerce fields");
