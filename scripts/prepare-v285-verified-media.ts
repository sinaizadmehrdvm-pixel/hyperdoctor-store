...
const partsDir = resolve("assets/catalog/v285/archive.parts");
...
const expectedPartCount = 8;

if (!existsSync(partsDir) || !existsSync(manifestPath)) throw new Error("Missing Version 285 verified-media bundle");
const partNames = readdirSync(partsDir).filter((name) => name.endsWith(".b64")).sort();
if (partNames.length !== expectedPartCount) throw new Error(`Expected ${expectedPartCount} Version 285 archive parts, found ${partNames.length}`);
const archiveBase64 = partNames.map((name) => readFileSync(join(partsDir, name), "utf8").trim()).join("");
...
const archiveEntries = execFileSync("tar", ["-tzf", "-"], { input: archive, encoding: "utf8" }).trim().split("\n").sort();
...
try {
  execFileSync("tar", ["-xzf", "-", "-C", staging], { input: archive, stdio: ["pipe", "inherit", "inherit"] });
