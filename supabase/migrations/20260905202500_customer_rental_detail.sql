...
begin
 ...
 select jsonb_build_object(
  ...
  'documents',coalesce((select jsonb_agg(jsonb_build_object('id',d.id,'documentType',d."documentType",'version',d.version,'sha256',d.sha256,'createdAt',d."createdAt",'snapshot',d.snapshot) order by d."documentType",d.version desc) from public."RentalDocumentSnapshot" d where d."rentalRequestId"=r.id),'[]'::jsonb)
 ) into result
...
revoke all on function public.customer_rental_detail(text,text) from public;
grant execute on function public.customer_rental_detail(text,text) to anon,authenticated;
