...
create index if not exists "LegacyCatalogProductIdentity_legacyCode_idx"
  on public."LegacyCatalogProductIdentity" (lower(btrim("legacyCode")));
create index if not exists "LegacyCatalogProductIdentity_barcode_idx"
  on public."LegacyCatalogProductIdentity" (lower(btrim(barcode))) where btrim(barcode)<>'';
create index if not exists "LegacyCatalogProductIdentity_name_idx"
  on public."LegacyCatalogProductIdentity" (lower(btrim("legacyName")));
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(x) order by x."sourceRow"),'[]'::jsonb) into v_result
  from (
    select id,"sourceFile","sourceRow","legacyCode","legacyName",barcode,"legacySku",brand,unit,
      "historicalPurchasePrice","historicalSellingPrice","historicalInitialStock","historicalMinStock","mainGroup","subGroup",status
    from public."LegacyCatalogProductIdentity"
    where status='REFERENCE' and (
      v_q='' or lower("legacyCode") like '%'||v_q||'%' or lower("legacyName") like '%'||v_q||'%'
      or lower(barcode) like '%'||v_q||'%' or lower("legacySku") like '%'||v_q||'%' or lower(brand) like '%'||v_q||'%'
    )
    order by "sourceRow"
    limit v_limit
  ) x;
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(x) order by x.score desc,x."sourceRow"),'[]'::jsonb) into v_result
  from (
    select i.id,i."sourceFile",i."sourceRow",i."legacyCode",i."legacyName",i.barcode,i.brand,i.unit,
      i."historicalPurchasePrice",i."historicalSellingPrice",i."historicalInitialStock",i."historicalMinStock",i."mainGroup",i."subGroup",
      (
        ...
        case when v_brand<>'' and lower(btrim(i.brand))=v_brand then 20 else 0 end
      ) as score
    from public."LegacyCatalogProductIdentity" i
    where i.status='REFERENCE'
  ) x
  where x.score>0
  order by x.score desc,x."sourceRow"
  limit v_limit;
  return v_result;
end $function$;
