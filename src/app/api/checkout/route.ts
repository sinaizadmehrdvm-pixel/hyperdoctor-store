...
function isSafePaymentRedirect(value:unknown):value is string{
 ...
 try{const url=new URL(value);return url.protocol==="https:"&&/^(?:www\.)?zarinpal\.com$/i.test(url.hostname);}catch{return false;}
}

export async function POST(request:Request){
 let raw:unknown;try{raw=await request.json();}catch{return NextResponse.json({error:"Invalid checkout payload"},{status:400});}
 const parsed=checkoutSchema.safeParse(raw);if(!parsed.success)return NextResponse.json({error:"Invalid checkout payload"},{status:400});
 ...
 try{
  ...
  return NextResponse.json({redirectUrl:payment.redirectUrl,orderNumber:order.orderNumber},{headers:{"Cache-Control":"no-store"}});
 }catch(error){
  ...
  console.error("[checkout] failed",error);return NextResponse.json({error:clientMessage},{status:502,headers:{"Cache-Control":"no-store"}});
 }
}
