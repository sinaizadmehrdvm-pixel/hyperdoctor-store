...
import { canAdminAccessPath } from "@/lib/admin-permissions";
...
const C:Record<AdminLocale,Record<string,string>>={
  fa:{title:"جستجوی پنل مدیریت",subtitle:"جستجوی همزمان فقط در بخش‌هایی که نقش شما اجازه دسترسی دارد",placeholder:"حداقل ۲ حرف وارد کنید",search:"جستجو",empty:"نتیجه‌ای در بخش‌های مجاز پیدا نشد.",hint:"برای شروع حداقل ۲ حرف وارد کنید.",PRODUCT:"محصول",ORDER:"سفارش",CUSTOMER:"مشتری",SERVICE:"خدمت",SUPPORT:"تیکت",ARTICLE:"مقاله"},
  ar:{title:"البحث في لوحة الإدارة",subtitle:"بحث موحد فقط ضمن الأقسام المسموح بها لدورك",placeholder:"أدخل حرفين على الأقل",search:"بحث",empty:"لم يتم العثور على نتائج ضمن الأقسام المسموحة.",hint:"أدخل حرفين على الأقل للبدء.",PRODUCT:"منتج",ORDER:"طلب",CUSTOMER:"عميل",SERVICE:"خدمة",SUPPORT:"تذكرة",ARTICLE:"مقال"},
  en:{title:"Admin search",subtitle:"Search only across sections permitted for your current role",placeholder:"Enter at least 2 characters",search:"Search",empty:"No results found in permitted sections.",hint:"Enter at least 2 characters to begin.",PRODUCT:"Product",ORDER:"Order",CUSTOMER:"Customer",SERVICE:"Service",SUPPORT:"Support ticket",ARTICLE:"Article"},
  tr:{title:"Yönetim paneli araması",subtitle:"Yalnızca mevcut rolünüzün erişebildiği bölümlerde arama yapın",placeholder:"En az 2 karakter girin",search:"Ara",empty:"İzin verilen bölümlerde sonuç bulunamadı.",hint:"Başlamak için en az 2 karakter girin.",PRODUCT:"Ürün",ORDER:"Sipariş",CUSTOMER:"Müşteri",SERVICE:"Hizmet",SUPPORT:"Destek talebi",ARTICLE:"Makale"}
};
...
export default async function AdminSearchPage({searchParams}:{searchParams:Promise<{q?:string}>}){
  const [{q=""},l,session]=await Promise.all([searchParams,currentAdminLocale(),requireAdminSession()]);
  const query=q.trim().slice(0,100),t=C[l];
  const raw=query.length>=2?await adminRpc<Item[]>("admin_global_search",{p_search:query}):[];
  const results=raw.filter(x=>canAdminAccessPath(session.role,x.href));
