...
async function ensureBwell(){return createVerifiedPackBatch(BWELL_2025_2026_SOURCE,BWELL_2025_2026_STARTER,{
  title:BWELL_TITLE,pricePolicy:"IGNORE",stockPolicy:"IGNORE",priceKind:"OTHER",currency:"IRT",
...
async function ensureJts(){return createVerifiedPackBatch(JTS_MAIN_CATALOG_SOURCE,JTS_MANUAL_WHEELCHAIR_STARTER,{
  title:JTS_TITLE,pricePolicy:"IGNORE",stockPolicy:"IGNORE",priceKind:"OTHER",currency:"IRT",
...
async function ensureHooshmand(){return createVerifiedPackBatch(HOOSHMAND_ORDIBEHESHT_1405_SOURCE,HOOSHMAND_ORDIBEHESHT_1405_MASTER,{
  title:HOOSHMAND_TITLE,pricePolicy:"HISTORICAL",stockPolicy:"IGNORE",priceKind:"CONSUMER",currency:"IRR",
...
export async function createStagingBatch(formData:FormData){
  ...
  const batch=await adminRpc<Batch>("admin_catalog_staging_create",{p_data:{sourceId,title,pricePolicy:String(formData.get("pricePolicy")||"IGNORE"),stockPolicy:String(formData.get("stockPolicy")||"IGNORE"),priceKind:String(formData.get("priceKind")||"OTHER"),currency:String(formData.get("currency")||"IRT"),notes:String(formData.get("notes")||"").trim()}});
  revalidatePath("/admin/products/staging");
...
export async function stageAllVerifiedLaunchPacks(){
  await Promise.all([ensureBwell(),ensureJts(),ensureHooshmand()]);
  revalidatePath("/admin/products/staging");
  revalidatePath("/admin/products/sources");
  redirect("/admin/products/staging?verified=ready");
}
...
export async function reviewStagingBatch(batchId:string,decision:"APPROVE"|"REJECT"){
  await adminRpc("admin_catalog_staging_review_batch",{p_batch_id:batchId,p_decision:decision});
  revalidatePath(`/admin/products/staging/${batchId}`);revalidatePath("/admin/products/staging");
}
...
export async function promoteStagingItem(batchId:string,itemId:string){
  ...
  revalidatePath(`/admin/products/staging/${batchId}`);revalidatePath("/admin/products/staging");revalidatePath("/admin/products");revalidatePath("/admin/commerce");revalidatePath("/","layout");
}

export async function promoteStagingBatch(batchId:string){
  await adminRpc("admin_catalog_staging_promote_batch",{p_batch_id:batchId});
  revalidatePath(`/admin/products/staging/${batchId}`);revalidatePath("/admin/products/staging");revalidatePath("/admin/products");revalidatePath("/admin/commerce");revalidatePath("/","layout");
}
