      ...
      <Container className="pt-5 sm:pt-8">
        <section className="stitch-home-stage relative mb-14 h-[58vh] min-h-[440px] overflow-hidden rounded-3xl border border-white/40 bg-black/5 shadow-xl sm:mb-20 sm:h-[65vh]">
          {heroSlides.map((slide, index) => (
            <div
              key={slide.accent}
              className={`stitch-hero-slide stitch-slide-${index + 1}`}
              style={{ backgroundImage: `url('${heroImages[index]}')`, opacity: index === 0 ? 1 : 0 }}
            >
              ...
              <div className="absolute inset-0 z-10 flex items-center justify-center px-5 text-center sm:px-10">
                <div className="max-w-2xl rounded-3xl p-4 text-white sm:p-8">
                  <h1 className="text-4xl font-black leading-tight sm:text-5xl">
                    {slide.title}<br />
                    <span className="text-[#82cfff]">{slide.accent}</span>
                  </h1>
                  <p className="mx-auto mt-5 max-w-xl text-sm leading-7 text-white/90 sm:text-lg sm:leading-8">{slide.body}</p>
                  <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
                    <Link href={slide.href} className="stitch-button stitch-button-primary">{slide.primary}<Arrow className="h-4 w-4" /></Link>
                    <Link href={index === 3 ? "/services" : index === 5 ? "/contact" : "/shop"} className="stitch-button stitch-button-ghost">{slide.secondary}</Link>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="stitch-logo-strip mb-14 sm:mb-20" aria-label="Brands">
          <div className="stitch-logo-track text-sm font-black tracking-wide text-[#43474f]">
            {["B.Well", "JTS", "BRISK", "Ehyagostar", "Hooshmand", "B.Well", "JTS", "BRISK", "Ehyagostar", "Hooshmand"].map((brand, i) => (
              <span key={`${brand}-${i}`} className="min-w-24 text-center opacity-75">{brand}</span>
            ))}
          </div>
        </section>
        ...
        <section className="mb-16 sm:mb-20">
          ...
          <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {categories.map((category, index) => {
              ...
              return (
                <Link key={category.id} href={`/shop/${category.slug}`} className="stitch-soft-card group flex min-h-32 flex-col items-center justify-center gap-3 p-4 text-center">
                  <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#d6e3ff] text-[#001736] group-hover:bg-[#001736] group-hover:text-white">
                    <Icon className="h-5 w-5" />
                  </span>
                  <span className="text-xs font-bold leading-6 text-[#181c1e] sm:text-sm">{localizedName(locale, category)}</span>
                </Link>
              );
            })}
          </div>
        ...
        <section className="mb-16 sm:mb-20" id="services">
          ...
          <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {services.slice(0, 4).map((service, index) => {
              ...
              return (
                <Link key={service.id} href={`/services/${service.slug}`} className="stitch-soft-card overflow-hidden">
                  <div className="relative h-36 bg-gradient-to-br from-[#d6e3ff] via-[#f1f4f7] to-[#c6e7ff]">
                    <div className="absolute inset-0 flex items-center justify-center"><Icon className="h-12 w-12 text-[#002b5b]" /></div>
                  </div>
                  <div className="p-5">
                    <h3 className="font-black text-[#181c1e]">{localizedName(locale, service)}</h3>
                    <p className="mt-2 line-clamp-2 text-xs leading-6 text-[#43474f]">{l(locale, "ارائه خدمت تخصصی با فرآیند استاندارد و پشتیبانی تیم هایپر دکتر.", "Specialist care with a standardized workflow and Hyper Doctor support.", "Standart süreç ve Hyper Doctor desteğiyle uzman hizmet.", "رعاية متخصصة بمسار معياري ودعم فريق هايبر دكتور.")}</p>
                    <span className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#002b5b]">{l(locale, "جزئیات بیشتر", "Learn more", "Detaylar", "المزيد")}<Arrow className="h-3.5 w-3.5" /></span>
                  </div>
                </Link>
              );
            })}
        ...
        {featured.length > 0 ? (
          <section className="mb-16 sm:mb-20">
            <div className="flex items-end justify-between gap-4">
              <h2 className="stitch-section-title text-start">{t("featuredTitle")}</h2>
              <Link href="/shop" className="text-sm font-bold text-[#002b5b]">{t("heroCtaShop")}</Link>
            </div>
            <div className="mt-7 grid grid-cols-2 gap-4 lg:grid-cols-4">{featured.map((product) => <ProductCard key={product.id} product={product} />)}</div>
          </section>
        ) : null}
        ...
        <section className="stitch-dark-feature mb-16 grid overflow-hidden lg:grid-cols-2 lg:items-stretch sm:mb-20" id="about">
          <div className="order-2 p-7 sm:p-10 lg:order-1 lg:p-14">
            <span className="inline-flex rounded-full bg-[#e80346] px-3 py-1 text-xs font-bold text-white">{l(locale, "درباره هایپر دکتر", "About Hyper Doctor", "Hyper Doctor hakkında", "عن هايبر دكتور")}</span>
            <h2 className="mt-5 text-2xl font-black sm:text-3xl">{l(locale, "تکنولوژی پزشکی، تجربه و مراقبت حرفه‌ای", "Medical technology, experience and professional care", "Medikal teknoloji, deneyim ve profesyonel bakım", "تقنية طبية وخبرة ورعاية احترافية")}</h2>
            <p className="mt-5 text-sm leading-8 text-white/75">{l(locale, "هایپر دکتر با تمرکز بر تجهیزات پزشکی، مراقبت تنفسی و خدمات تخصصی، مسیر انتخاب تا پشتیبانی را در یک تجربه یکپارچه ارائه می‌کند.", "Hyper Doctor brings medical equipment, respiratory care and specialist services into one continuous experience from selection to support.", "Hyper Doctor; medikal ekipman, solunum bakımı ve uzman hizmetleri seçimden desteğe tek bir deneyimde birleştirir.", "يجمع هايبر دكتور المعدات الطبية والرعاية التنفسية والخدمات المتخصصة في تجربة متكاملة من الاختيار حتى الدعم.")}</p>
            <div className="mt-7 flex items-center gap-4"><span className="text-4xl font-black text-[#82cfff]">15+</span><span className="text-xs leading-6 text-white/70">{l(locale, "سال تجربه در حوزه تجهیزات و خدمات سلامت", "years of healthcare equipment and service experience", "yıllık sağlık ekipmanı ve hizmet deneyimi", "سنة من الخبرة في المعدات والخدمات الصحية")}</span></div>
          </div>
          <div className="order-1 min-h-64 bg-[radial-gradient(circle_at_50%_40%,rgba(0,157,216,.35),transparent_45%),linear-gradient(135deg,#002f44,#001736)] lg:order-2">
            <div className="flex h-full min-h-64 items-center justify-center"><Stethoscope className="h-24 w-24 text-[#82cfff]" /></div>
          </div>
        </section>

        <section className="mb-16 sm:mb-20">
          <h2 className="stitch-section-title">{l(locale, "تضمین کیفیت و اصالت تجهیزات", "Quality & Authenticity Assurance", "Kalite ve Orijinallik Güvencesi", "ضمان الجودة والأصالة")}</h2>
          <p className="mx-auto mt-2 max-w-2xl text-center text-sm text-[#747780]">{l(locale, "استانداردهای حرفه‌ای برای انتخاب، تحویل و پشتیبانی تجهیزات پزشکی", "Professional standards for equipment selection, delivery and support", "Ekipman seçimi, teslimat ve destek için profesyonel standartlar", "معايير احترافية لاختيار المعدات وتسليمها ودعمها")}</p>
          <div className="mt-7 grid grid-cols-2 gap-3 lg:grid-cols-4">
            {[
              [ShieldCheck, l(locale, "اصالت و کنترل کیفیت", "Authenticity & quality", "Orijinallik ve kalite", "الأصالة والجودة")],
              [Award, l(locale, "گارانتی معتبر", "Valid warranty", "Geçerli garanti", "ضمان معتمد")],
              [PackageCheck, l(locale, "تحویل ایمن", "Safe delivery", "Güvenli teslimat", "توصيل آمن")],
              [Wrench, l(locale, "پشتیبانی تخصصی", "Specialist support", "Uzman destek", "دعم متخصص")],
            ].map(([Icon, label]) => {
              const QualityIcon = Icon as typeof ShieldCheck;
              return <div key={String(label)} className="stitch-soft-card flex flex-col items-center gap-3 p-6 text-center"><QualityIcon className="h-7 w-7 text-[#002b5b]" /><strong className="text-sm">{label as string}</strong></div>;
            })}
          </div>
        </section>

        <section className="mb-16 sm:mb-20" id="articles">
          <h2 className="stitch-section-title">{l(locale, "مقالات علمی و تخصصی", "Scientific & Specialist Articles", "Bilimsel ve Uzman Makaleler", "مقالات علمية ومتخصصة")}</h2>
          <div className="mt-7 grid gap-4 md:grid-cols-3">
            {articleCards.map((title, index) => (
              <article key={title} className="stitch-soft-card overflow-hidden">
                <div className="flex h-36 items-center justify-center bg-gradient-to-br from-[#e5e8eb] to-[#d6e3ff]"><BookOpen className="h-10 w-10 text-[#002b5b]" /></div>
                <div className="p-5"><span className="text-[11px] font-bold text-[#ba0036]">HYPER DOCTOR JOURNAL</span><h3 className="mt-2 font-black leading-7">{title}</h3><p className="mt-3 text-xs leading-6 text-[#747780]">{l(locale, "مطالب آموزشی و علمی برای استفاده بهتر و ایمن‌تر از تجهیزات و خدمات سلامت.", "Practical clinical knowledge for safer, better use of healthcare equipment and services.", "Sağlık ekipmanı ve hizmetlerini daha güvenli kullanmak için pratik klinik bilgi.", "معرفة سريرية عملية لاستخدام أكثر أماناً وفعالية للمعدات والخدمات الصحية.")}</p></div>
              </article>
            ))}
          </div>
        </section>

        <section className="stitch-dark-feature mb-16 flex flex-col items-center justify-between gap-6 p-7 text-center sm:flex-row sm:p-10 sm:text-start sm:mb-20">
          <div><span className="text-xs font-bold text-[#82cfff]">VITALIS • HYPER DOCTOR</span><h2 className="mt-2 text-xl font-black sm:text-2xl">{l(locale, "باشگاه مشتریان هایپر دکتر", "Hyper Doctor Customer Club", "Hyper Doctor Müşteri Kulübü", "نادي عملاء هايبر دكتور")}</h2><p className="mt-2 text-sm text-white/70">{l(locale, "خدمات، پیگیری و مزایای اختصاصی را در یک مسیر یکپارچه مدیریت کنید.", "Manage services, follow-up and member benefits in one place.", "Hizmetleri, takibi ve üye avantajlarını tek yerde yönetin.", "أدر الخدمات والمتابعة ومزايا العضوية في مكان واحد.")}</p></div>
          <Link href="/account" className="stitch-button stitch-button-primary shrink-0">{l(locale, "ورود به باشگاه", "Open customer area", "Müşteri alanına gir", "دخول منطقة العملاء")}<Arrow className="h-4 w-4" /></Link>
        </section>

        <section className="mb-4" id="faq">
          <h2 className="stitch-section-title">{l(locale, "سوالات متداول", "Frequently Asked Questions", "Sık Sorulan Sorular", "الأسئلة الشائعة")}</h2>
          <p className="mx-auto mt-2 max-w-xl text-center text-sm text-[#747780]">{l(locale, "پاسخ به رایج‌ترین سوالات درباره تجهیزات و خدمات هایپر دکتر", "Common questions about Hyper Doctor equipment and services", "Hyper Doctor ekipman ve hizmetleri hakkında sık sorulanlar", "أسئلة شائعة حول معدات وخدمات هايبر دكتور")}</p>
          <div className="mx-auto mt-7 max-w-3xl space-y-3">
            {faqs.map((question, index) => (
              <details key={question} className="stitch-glass-card rounded-xl px-5 py-4 open:bg-white">
                <summary className="cursor-pointer list-none text-sm font-bold text-[#181c1e]">{question}</summary>
                <p className="mt-3 border-t border-[#c4c6d0]/40 pt-3 text-xs leading-6 text-[#43474f]">{index === 0 ? l(locale, "از طریق بخش تماس یا رزرو مشاوره، درخواست خود را ثبت کنید تا کارشناس بر اساس نیاز شما راهنمایی کند.", "Use contact or consultation booking and our specialist will guide you based on your needs.", "İletişim veya danışmanlık rezervasyonu üzerinden talebinizi iletin; uzmanımız ihtiyacınıza göre yönlendirsin.", "أرسل طلبك عبر التواصل أو حجز الاستشارة وسيقوم المختص بإرشادك وفق احتياجاتك.") : l(locale, "جزئیات این خدمت بر اساس نوع تجهیزات یا درخواست شما توسط تیم پشتیبانی اعلام می‌شود.", "Details are confirmed by our support team based on the equipment or service requested.", "Ayrıntılar ekipman veya hizmet talebinize göre destek ekibimizce doğrulanır.", "يؤكد فريق الدعم التفاصيل وفق نوع المعدات أو الخدمة المطلوبة.")}</p>
              </details>
            ))}
          </div>
        </section>
