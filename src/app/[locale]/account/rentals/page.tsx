  ...
  <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
   ...
   <section className="mt-6 space-y-4">
    {rentals.map(r=>{const currency=r.branch?.currency||"";const start=r.approvedStartDate||r.preferredStartDate;const end=r.approvedEndDate||r.preferredEndDate;const state=r.lifecycleState||r.requestStatus;return <article key={r.rentalRequestId} className="rounded-[1.75rem] border border-white bg-white p-5 shadow-[0_14px_38px_rgba(0,23,54,.05)] sm:p-6">
     ...
     <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-[#edf0f3] pt-4"><DocumentBadge ok={r.hasContract} text={pick(locale,"قرارداد","Contract","Sözleşme","العقد")}/><DocumentBadge ok={r.hasSettlement} text={pick(locale,"گزارش تسویه","Settlement report","Uzlaşma raporu","تقرير التسوية")}/><span className="flex-1"/><Link href={`/${locale}/rental/track`} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-[#b9dff0] px-3 text-xs font-black text-[#002b5b]"><Search className="h-4 w-4"/>{pick(locale,"رهگیری و چاپ سند","Track & print documents","Takip ve belge yazdır","التتبع وطباعة المستندات")}</Link></div>
    </article>})}
