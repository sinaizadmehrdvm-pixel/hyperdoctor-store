...
export default async function AboutPage() {
  ...
  const fa = {
    ...
    body: "هایپر دکتر با تکیه بر تجربه تخصصی در تجهیزات پزشکی و خدمات سلامت، راهکارهای تنفسی و مراقبتی را با دقت، پاسخ‌گویی و پشتیبانی واقعی ارائه می‌کند.",
    cta: "کشف مأموریت ما", values: "ارزش‌های محوری ما", valuesSub: "مهندسی دقیق و طراحی انسان‌محور، پیشران آینده تخصص‌های پزشکی است.",
  ...
  const tr = { ...en, title:"Solunum bakımında öncü. Sağlık hizmetlerinde mükemmellik.", body:"Hyper Doctor; medikal cihaz deneyimini güvenilir solunum ve bakım hizmetleriyle birleştirir.", cta:"Misyonumuzu keşfedin", values:"Temel değerlerimiz", valuesSub:"Hassas mühendislik ve insan odaklı tasarım uzman bakımın geleceğini şekillendirir." };
  const ar = { ...en, title:"ريادة في رعاية الجهاز التنفسي. تميز في الخدمات الصحية.", body:"تجمع Hyper Doctor بين خبرة المعدات الطبية وخدمات التنفس والرعاية الموثوقة.", cta:"اكتشف مهمتنا", values:"قيمنا الأساسية", valuesSub:"الهندسة الدقيقة والتصميم المتمحور حول الإنسان يقودان مستقبل الرعاية المتخصصة." };
    ...
    <section className="relative min-h-[590px] overflow-hidden bg-[#001736] text-white">
      ...
      <Container className="relative z-10 flex min-h-[590px] items-center justify-center text-center">
        <div className="max-w-4xl py-16">
          <h1 className="text-4xl font-black leading-[1.45] drop-shadow-lg sm:text-6xl">{c.title}</h1>
          <p className="mx-auto mt-6 max-w-3xl text-sm leading-8 text-white/78 sm:text-base">{c.body}</p>
          <Link href="#values" className="mt-8 inline-flex min-h-12 items-center rounded-full border border-white/35 bg-white/10 px-7 text-sm font-black text-white backdrop-blur hover:bg-white/15">{c.cta}</Link>
        </div>
      </Container>
    </section>

    <section id="values" className="py-16 sm:py-20"><Container>
      <div className="mx-auto max-w-3xl text-center"><h2 className="text-3xl font-black text-[#001736] sm:text-4xl">{c.values}</h2><p className="mt-4 text-sm leading-7 text-[#5f6570]">{c.valuesSub}</p></div>
      <div className="mt-10 grid gap-5 md:grid-cols-3">
        {c.cards.map(([title, body], index) => { const Icon = icons[index]; const dark = index === 1; return <article key={title} className={`relative overflow-hidden rounded-3xl border p-7 shadow-[0_16px_45px_rgba(0,23,54,.06)] ${dark ? "border-[#001736] bg-[#001736] text-white" : "border-[#e0e5eb] bg-white text-[#001736]"}`}><span className={`flex h-12 w-12 items-center justify-center rounded-2xl ${dark ? "bg-white/8 text-[#ff3971]" : "bg-[#eef4fb] text-[#009dd8]"}`}><Icon className="h-6 w-6" /></span><h3 className="mt-7 text-xl font-black">{title}</h3><p className={`mt-4 text-sm leading-8 ${dark ? "text-white/68" : "text-[#5f6570]"}`}>{body}</p></article> })}
      </div>
    </Container></section>
  </main>;
}
