...
type SP={sort?:string;q?:string;brand?:string;taxonomy?:string|string[];use?:string};
export async function generateMetadata({searchParams}:{searchParams:Promise<SP>}):Promise<Metadata>{const sp=await searchParams;const faceted=Boolean(sp.q||sp.sort||sp.brand||sp.taxonomy||sp.use);return faceted?{robots:{index:false,follow:true}}:{}}
