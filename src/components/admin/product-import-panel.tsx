    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-border bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <Link
            href="/data/product-import-template.csv"
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-border bg-muted-bg/60 p-6 text-center">
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-3">
            <button type="button" onClick={() => inputRef.current?.click()} className="vitalis-focus min-h-10 rounded-xl border border-border bg-white px-4 text-xs font-bold text-foreground hover:bg-muted-bg">
              انتخاب فایل
            </button>
      ...
      {result ? (
        <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 text-center">
            <Stat label="جدید" value={result.created} />
            <Stat label="به‌روزرسانی" value={result.updated} />
            <Stat label="خطا" value={result.failed} />
          </div>
          {result.errors?.length ? (
            <div className="mt-5 max-h-72 overflow-auto rounded-xl border border-amber-200 bg-white p-3">
              <p className="mb-2 flex items-center gap-1.5 text-xs font-black text-amber-800"><AlertTriangle className="h-4 w-4" aria-hidden="true" />ردیف‌های نیازمند اصلاح</p>
              <ul className="space-y-1 text-xs text-muted">
                {result.errors.map((item, index) => <li key={`${item.line}-${index}`}>ردیف {item.line} {item.sku ? `(${item.sku})` : ""}: {item.error}</li>)}
              </ul>
            </div>
          ) : null}
        </section>
