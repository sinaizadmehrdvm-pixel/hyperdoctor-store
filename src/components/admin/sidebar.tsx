...
export function AdminSidebar() {
  ...
  return <nav className="flex flex-col gap-1 p-4">{items.map((item) => {
    const active = item.href === "/admin" ? pathname === "/admin" : item.href === "/admin/products" ? pathname === "/admin/products" || /^\/admin\/products\/(new|[^/]+)$/.test(pathname) : pathname.startsWith(item.href);
    const Icon = item.icon;
    return <Link key={item.href} href={item.href} className={cn("flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition-colors", active ? "bg-primary text-white shadow-sm" : "text-foreground hover:bg-muted-bg")}><Icon className="h-4.5 w-4.5" aria-hidden="true" />{item.label}</Link>;
  })}</nav>;
}
