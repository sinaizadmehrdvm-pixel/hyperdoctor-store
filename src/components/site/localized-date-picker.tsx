...
import {appLocale,calendarConfig,calendarNumericParts,calendarParts,formatLocalizedDate,localISODate,type AppLocale} from "@/lib/calendar";
...
const COPY:Record<AppLocale,{today:string;previous:string;next:string;choose:string}>={fa:{today:"امروز",previous:"ماه قبل",next:"ماه بعد",choose:"انتخاب تاریخ"},tr:{today:"Bugün",previous:"Önceki ay",next:"Sonraki ay",choose:"Tarih seç"},en:{today:"Today",previous:"Previous month",next:"Next month",choose:"Choose date"},ar:{today:"اليوم",previous:"الشهر السابق",next:"الشهر التالي",choose:"اختر التاريخ"}};
...
function moveCalendarMonth(date:Date,locale:AppLocale,delta:number){let cursor=calendarMonthStart(date,locale);if(delta>0){const current=calendarNumericParts(cursor,locale);for(let i=1;i<=35;i++){const candidate=addDays(cursor,i),parts=calendarNumericParts(candidate,locale);if(parts&&current&&(parts.month!==current.month||parts.year!==current.year))return candidate;}}else{cursor=addDays(cursor,-1);return calendarMonthStart(cursor,locale)}return cursor}
function monthGrid(anchor:Date,locale:AppLocale){const first=calendarMonthStart(anchor,locale);const offset=(first.getDay()-weekStart(locale)+7)%7;const start=addDays(first,-offset);return Array.from({length:42},(_,i)=>addDays(start,i))}
...
function weekdays(locale:AppLocale){const base=new Date(2024,0,7,12),start=weekStart(locale);return Array.from({length:7},(_,i)=>{const date=addDays(base,(start+i)%7);return new Intl.DateTimeFormat(calendarConfig(locale).locale,{weekday:"short"}).format(date)})}
...
export function LocalizedDatePicker({value,onChange,locale:rawLocale,min,max,disabled,id,className=""}:Props){
 ...
 useEffect(()=>{if(!open)return;const close=(event:PointerEvent)=>{if(root.current&&!root.current.contains(event.target as Node))setOpen(false)};const key=(event:KeyboardEvent)=>{if(event.key==="Escape")setOpen(false)};document.addEventListener("pointerdown",close);document.addEventListener("keydown",key);return()=>{document.removeEventListener("pointerdown",close);document.removeEventListener("keydown",key)}},[open]);
 const days=useMemo(()=>monthGrid(anchor,locale),[anchor,locale]),names=useMemo(()=>weekdays(locale),[locale]),anchorParts=calendarNumericParts(anchor,locale);
 const blocked=(date:Date)=>(minDate&&date<minDate)||(maxDate&&date>maxDate);
 const moveMonth=(delta:number)=>setAnchor(current=>moveCalendarMonth(current,locale,delta));
 ...
 return <div ref={root} className={`relative ${className}`} dir={dir}>
  ...
  {open?<div role="dialog" aria-modal="false" aria-label={copy.choose} className="absolute z-50 mt-2 w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-[#dfe4ea] bg-white p-4 shadow-[0_24px_60px_rgba(0,23,54,.18)]">
