...
export async function Header() {
  ...
  const mobileItems=[...navItems,{href:customer?"/account":"/account/login",label:customer?l(locale,"حساب من","My account","Hesabım","حسابي"):l(locale,"ورود / ثبت‌نام","Sign in / Register","Giriş / Kayıt","دخول / تسجيل")}];
    ...
    <header className="sticky top-0 z-50 border-b border-[#c4c6d0]/35 bg-white/85 text-[#001736] shadow-[0_6px_24px_rgba(0,23,54,0.06)] backdrop-blur-xl">
      <div className="vitalis-container flex min-h-18 items-center justify-between gap-3 py-2.5">
        ...
        <div className="flex items-center gap-1.5">
          <Link href="/contact" className="hidden min-h-10 items-center rounded-full border border-[#c4c6d0]/60 bg-white px-3 text-xs font-bold text-[#001736] transition hover:bg-[#f1f4f7] lg:flex">{l(locale, "مشاوره رایگان", "Free consultation", "Ücretsiz danışmanlık", "استشارة مجانية")}</Link>
