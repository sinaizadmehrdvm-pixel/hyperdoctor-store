...
export function SectionRenderer({ section, locale, viewport = "desktop", editable = false, selected = false, onSelect, onInlineChange, onRequestImage }: Props) {
  ...
  const style = { background: String(s.background || "#ffffff"), color: String(s.foreground || "#001736"), paddingTop: px(s.paddingY, 64), paddingBottom: px(s.paddingY, 64), minHeight: section.type === "hero" ? px(s.minHeight, 420) : undefined, borderRadius: px(s.radius, 0) };
  ...
  if (section.type === "hero") {
    ...
    return <section {...common}><div className={inner} style={{ maxWidth, paddingInline: "24px", justifyContent: "center" }}>
      ...
      {(button || editable) ? editable ? <span className="mt-7 inline-flex min-h-12 items-center rounded-xl bg-[#001736] px-6 text-sm font-black text-white"><EditableText value={button} editable onChange={inline("buttonLabel")}>{button}</EditableText></span> : <Link href={String(section.content.buttonHref || "/")} className="mt-7 inline-flex min-h-12 items-center rounded-xl bg-[#001736] px-6 text-sm font-black text-white">{button}</Link> : null}
    </div></section>;
  }
  ...
  if (section.type === "imageText") {
    ...
    return <section {...common}><div className="mx-auto grid w-full items-center gap-10 md:grid-cols-2" style={{ maxWidth, paddingInline: "24px" }}>
      ...
      <div className={imageFirst ? "md:order-1" : ""}><ReplaceableImage src={String(section.content.imageUrl || "")} alt={localize(section.content.imageAlt, locale)} editable={editable} onReplace={()=>onRequestImage?.({ kind: "section" })} className="aspect-[4/3] w-full rounded-[2rem] object-cover"/></div>
    </div></section>;
  }
  ...
  if (section.type === "cards") {
    ...
    return <section {...common}><div className="mx-auto w-full" style={{ maxWidth, paddingInline: "24px" }}>{(title || editable) ? <h2 className="mb-8 text-3xl font-black"><EditableText value={title} editable={editable} onChange={inline("title")}>{title}</EditableText></h2> : null}<div className={`grid gap-5 ${cols}`}>{cards.map(card=><div key={card.id} className="overflow-hidden rounded-[1.5rem] border border-black/10 bg-white/80 shadow-sm">{card.imageUrl || editable ? <ReplaceableImage src={card.imageUrl} alt="" editable={editable} onReplace={()=>onRequestImage?.({ kind: "card", cardId: card.id })} className="aspect-[16/10] w-full object-cover"/> : null}<div className="p-5"><h3 className="text-lg font-black"><EditableText value={localize(card.title, locale)} editable={editable} onChange={inline(`card:${card.id}:title`)}>{localize(card.title, locale)}</EditableText></h3><p className="mt-2 text-sm leading-7 opacity-70"><EditableText value={localize(card.body, locale)} editable={editable} onChange={inline(`card:${card.id}:body`)}>{localize(card.body, locale)}</EditableText></p>{card.href && !editable ? <Link href={card.href} className="mt-4 inline-flex text-xs font-black underline">Open</Link> : null}</div></div>)}</div></div></section>;
  }
  ...
  return <section {...common}><div className={inner} style={{ maxWidth, paddingInline: "24px" }}><h2 className="text-3xl font-black sm:text-4xl"><EditableText value={title} editable={editable} onChange={inline("title")}>{title}</EditableText></h2><p className="mt-4 max-w-2xl text-base leading-8 opacity-75"><EditableText value={body} editable={editable} onChange={inline("body")}>{body}</EditableText></p>{button || editable ? editable ? <span className="mt-6 inline-flex min-h-12 items-center rounded-xl bg-[#e80346] px-6 text-sm font-black text-white"><EditableText value={button} editable onChange={inline("buttonLabel")}>{button}</EditableText></span> : <Link href={String(section.content.buttonHref || "/")} className="mt-6 inline-flex min-h-12 items-center rounded-xl bg-[#e80346] px-6 text-sm font-black text-white">{button}</Link> : null}</div></section>;
}
